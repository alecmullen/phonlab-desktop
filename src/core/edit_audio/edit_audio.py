...
class EditAudio(UseCase[EditResult | None]):
    ...
    def _resample_signal(self, clip_x: np.ndarray, clip_fs: int) -> AudioSignal:
        ...
        return resample_poly(clip_x, up=self._channel.fs // cd, down=clip_fs // cd).astype(self._channel.x.dtype)
    ...
    def invoke(self):
        ...
        if self._edit_command.type == EditCommandType.PASTE:
            ...
            clip_x = self._resample_signal(x, fs, self._channel.fs) if self._channel.fs != fs else x

            start_idx = int(np.clip(self._edit_command.start_time * fs, 0, len(self._channel.x)))
            if settings.cut_and_paste_at_zero_crossings: 
                start_idx = self._nearest_zero_crossing_boundary(start_idx)

            new_x = np.concatenate([self._channel.x[:start_idx], clip_x, self._channel.x[start_idx:]])
            return EditResult(
                new_channel=AudioSignal(new_x, self._channel.fs)
            )
